        ...
        **overrides,
        ...
        obsolete = sorted(
            ...
            for key in (
                "pim_banks_per_mpu",
                "pim_slot_cost",
                "pim_mac_latency_scale",
                "pim_incremental_energy_scale",
            )
            if key in overrides
        )
        if obsolete:
            raise ValueError(
                "LPDDR6PIM obsolete compatibility field(s): "
                + ", ".join(obsolete)
                + "; use canonical LPDDR6PIM fields"
            )
        ...
        if pim_mac_execution_model == "shared_mpu_serial":
            raise ValueError(
                "LPDDR6PIM obsolete execution model 'shared_mpu_serial'; "
                "use 'shared_block_serial'"
            )
