###############################################################################
# AUTO-GENERATED FILE — DO NOT EDIT
#
# Generated by: python -m ramulator codegen
# Regenerate:   python -m ramulator codegen
###############################################################################
from .bho3 import BHO3
from .external import External
from .latency_throughput_trace import LatencyThroughputTrace
from .load_store_trace import LoadStoreTrace
from .lpddr5_pim_concrete_trace import LPDDR5PIMConcreteTrace
from .lpddr6_pim_concrete_trace import LPDDR6PIMConcreteTrace
from .read_write_trace import ReadWriteTrace
from .simple_o3 import SimpleO3
from .structured_workload_surrogate_trace import StructuredWorkloadSurrogateTrace

__all__ = ['BHO3', 'External', 'LPDDR5PIMConcreteTrace', 'LPDDR6PIMConcreteTrace', 'LatencyThroughputTrace', 'LoadStoreTrace', 'ReadWriteTrace', 'SimpleO3', 'StructuredWorkloadSurrogateTrace']
